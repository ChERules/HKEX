	...
	for line in qpage:
		...
		# read the first line of each quotation
			...
			if line.startswith('------'):
				...
				st = 'd'
				break
#			if line[27:].find('TRADING'): continue
			q = read_line(line)
			#print(q[0])
			if len(q[0]) > 0:
				code = q[0]
				...
				csvout = csvout + ask + ',' + bid + ',' + tunovr + ',' + traded + '\n'
				qout.write(csvout)
	return(senum)
...
def fexist(name, path):
	# varify if a file with name "name" already exist in the directory "path"
	flist = []
	for file in os.listdir(path):
	...
	for line in qpage:
		...
		# read the first line of each quotation
			...
			if len(q[0]) > 0:
				...
				low   = q[5]
				tunovr = q[6]
			outf.write(code+','+name+','+cur+','+high+','+low+','+close+','+prv+','+ask+','+bid+','+tunovr+','+traded+'\n')
